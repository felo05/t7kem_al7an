...
abstract class BaseMarksFormModel {
  ...
  double _parseDouble(String value) => double.tryParse(value.trim()) ?? double.nan;
...
class Kg1FormModel extends BaseMarksFormModel {
  Kg1FormModel({
	...
	TextEditingController? slokController,
		...
		controllers1 = controllers1 ?? List.generate(3, (_) => TextEditingController()),
		controllers2 = controllers2 ?? List.generate(3, (_) => TextEditingController()),
		controllers3 = controllers3 ?? List.generate(3, (_) => TextEditingController()),
		controllers4 = controllers4 ?? List.generate(3, (_) => TextEditingController()),
  ...
  bool validate() {
	...
	final controllerGroups = [controllers1, controllers2, controllers3, controllers4];
	for (var i = 0; i < controllerGroups.length; i++) {
	  ...
	  for (var j = 0; j < controllerGroups[i].length; j++) {
		if (!_withinRange(controllerGroups[i][j].text, validations[i][j])) return false;
	  }
	}
	return true;
  }
...
class Kg2FormModel extends BaseMarksFormModel {
  Kg2FormModel({
	...
	TextEditingController? slokController,
		...
		controllers3 = controllers3 ?? List.generate(3, (_) => TextEditingController()),
		controllers4 = controllers4 ?? List.generate(3, (_) => TextEditingController()),
		controllers5 = controllers5 ?? List.generate(3, (_) => TextEditingController()),
		controllers6 = controllers6 ?? List.generate(3, (_) => TextEditingController()),
		totalController = totalController ?? TextEditingController(),
		slokController = slokController ?? TextEditingController(text: '10');
  ...
  bool validate() {
	...
	if (!_withinRange(slokController.text, 10)) return false;
	final controllerGroups = [controllers1, controllers2, controllers3, controllers4, controllers5, controllers6];
...
class Talta1FormModel extends BaseMarksFormModel {
  Talta1FormModel({
	...
	List<bool>? bool4,
		...
		controllers1 = controllers1 ?? List.generate(4, (_) => TextEditingController()),
		controllers2 = controllers2 ?? List.generate(4, (_) => TextEditingController()),
		controllers3 = controllers3 ?? List.generate(4, (_) => TextEditingController()),
		controllers4 = controllers4 ?? List.generate(4, (_) => TextEditingController()),
		totalController = totalController ?? TextEditingController(),
	  ...
	  builder: (context, setState) {
		return Column(
		  ...
		  children: [
			_churchHeader(),
			_sectionGap(),
			MarksFormFields.taltaForm(al7anList[0], controllers1, bool1, (index, value) {
			  setState(() => bool1[index] = value ?? false);
			}),
			_sectionGap(),
			MarksFormFields.taltaForm(al7anList[1], controllers2, bool2, (index, value) {
			  setState(() => bool2[index] = value ?? false);
			}),
			_sectionGap(),
			MarksFormFields.taltaForm(al7anList[2], controllers3, bool3, (index, value) {
			  setState(() => bool3[index] = value ?? false);
			}),
			_sectionGap(),
			MarksFormFields.taltaForm(al7anList[3], controllers4, bool4, (index, value) {
  ...
  bool validate() {
	...
	for (final group in [controllers1, controllers2, controllers3, controllers4]) {
	  for (var i = 0; i < group.length; i++) {
		if (!_withinRange(group[i].text, limits[i])) return false;
	  }
	}
...
class Talta2FormModel extends BaseMarksFormModel {
  Talta2FormModel({
	...
	List<bool>? bool6,
		...
		controllers3 = controllers3 ?? List.generate(4, (_) => TextEditingController()),
		controllers4 = controllers4 ?? List.generate(4, (_) => TextEditingController()),
		controllers5 = controllers5 ?? List.generate(4, (_) => TextEditingController()),
		controllers6 = controllers6 ?? List.generate(4, (_) => TextEditingController()),
		totalController = totalController ?? TextEditingController(),
		taksController = taksController ?? TextEditingController(),
		slokController = slokController ?? TextEditingController(text: '10'),
		bool1 = bool1 ?? List.generate(3, (_) => false),
	  ...
	  builder: (context, setState) {
		return Column(
		  ...
		  children: [
			...
			_sectionGap(),
			MarksFormFields.taltaForm(al7anList[4], controllers5, bool5, (index, value) {
			  setState(() => bool5[index] = value ?? false);
			}),
			_sectionGap(),
			MarksFormFields.taltaForm(al7anList[5], controllers6, bool6, (index, value) {
  ...
  bool validate() {
	...
	final limits = [20.0, 10.0, 10.0, 10.0];
	for (final group in [controllers1, controllers2, controllers3, controllers4, controllers5, controllers6]) {
	  for (var i = 0; i < group.length; i++) {
		if (!_withinRange(group[i].text, limits[i])) return false;
	  }
	}
...
class MohobenIndividualFormModel extends BaseMarksFormModel {
	  ...
	  builder: (context, setState) {
		return Column(
		  ...
		  children: [
			_churchHeader(),
			_sectionGap(),
			MarksFormFields.mohobenIndividualForm(al7anList[0], controllers1, bool1, (index, value) {
			  setState(() => bool1[index] = value ?? false);
			}, level),
			_sectionGap(),
			MarksFormFields.mohobenIndividualForm(al7anList[1], controllers2, bool2, (index, value) {
			  setState(() => bool2[index] = value ?? false);
			}, level),
			_sectionGap(),
			MarksFormFields.mohobenIndividualForm(al7anList[2], controllers3, bool3, (index, value) {
			  setState(() => bool3[index] = value ?? false);
			}, level),
		  ],
		);
  ...
  @override
  void dispose() {
	for (final controller in [...controllers1, ...controllers2, ...controllers3]) {
	  controller.dispose();
	}
  }
...
class MohobenGroupFormModel extends BaseMarksFormModel {
	...
	List<bool>? bool3,
			...
			: level == 1
				...
				: level == 2
					? Al7an.talta3
					: Al7an.khamsa3),
		controllers1 = controllers1 ?? List.generate(5, (_) => TextEditingController()),
		controllers2 = controllers2 ?? List.generate(5, (_) => TextEditingController()),
		controllers3 = controllers3 ?? List.generate(5, (_) => TextEditingController()),
		totalController = totalController ?? TextEditingController(),
		slokController = slokController ?? TextEditingController(text: '10'),
		bool1 = bool1 ?? List.generate(3, (_) => false),
		bool2 = bool2 ?? List.generate(3, (_) => false),
	  ...
	  builder: (context, setState) {
		return Column(
		  ...
		  children: [
			_churchHeader(),
			_sectionGap(),
			MarksFormFields.mohobenGroupForm(al7anList[0], controllers1, bool1, (index, value) {
			  setState(() => bool1[index] = value ?? false);
			}, level),
			_sectionGap(20),
			MarksFormFields.mohobenGroupForm(al7anList[1], controllers2, bool2, (index, value) {
			  setState(() => bool2[index] = value ?? false);
			}, level),
			_sectionGap(20),
			MarksFormFields.mohobenGroupForm(al7anList[2], controllers3, bool3, (index, value) {
			  setState(() => bool3[index] = value ?? false);
			}, level),
			_sectionGap(),
			MarksFormFields.total(totalController),
  ...
  @override
  bool validate() {
	if (totalController.text.trim().isEmpty || slokController.text.trim().isEmpty) return false;
	if (!_withinRange(slokController.text, 10)) return false;
	final controllerGroups = [controllers1, controllers2, controllers3];
	for (final group in controllerGroups) {
	  if (_isEmptyControllers(group)) return false;
  ...
  @override
  void dispose() {
	for (final controller in [...controllers1, ...controllers2, ...controllers3, totalController, slokController]) {
	  controller.dispose();
	}
  }
