...
class MohobenIndividual extends StatefulWidget {
  const MohobenIndividual({super.key, required this.level, required this.churchName});
  final int level;
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              MarksFormFields.mohobenIndividualForm(al7anList[0], controllers1, bool1,
                      (index, value) {
                    setState(() {
              ...
              const SizedBox(height: 10),
              MarksFormFields.mohobenIndividualForm(al7anList[1], controllers2, bool2,
                      (index, value) {
                    setState(() {
              ...
              const SizedBox(height: 10),
              MarksFormFields.mohobenIndividualForm(al7anList[2], controllers3, bool3,
                      (index, value) {
                    setState(() {
              ...
              MarksFormFields.submitButton( onPressed: ()async {
                ...
                sum+= bool3[0] ? 1 : 0;
                sum+= bool3[1] ? 1 : 0;
                factor=(sum>=50&&sum<=53)?1.01:
                (sum>=54&&sum<=56)?1.02:
                (sum>=57&&sum<=59)?1.05:
                (sum>=60&&sum<=61)?1.07:1;
                ...
                await fireStore
                    .collection("${widget.level == 0
                    ? Firebase.kgg
                    : widget.level == 1
                    ? Firebase.olag
                    : widget.level == 2
                    ? Firebase.taltag
                    : Firebase.khamsag}result").add(estmara).then((val){Navigator.pop(context);});
              }),
            ],
          ),
        ),
